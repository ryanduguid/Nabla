...
That claim describes the v3.0.0 baseline. Post-v3.0.0 passes start from the committed `ozzit.xlsx` and `src/` rather than from upstream. The committed input those later passes were written against is 439,209 bytes, sha256 `26a3e6246ff3d849bb2eb9295b39900682a3ec69dd8475005278de7ad22ef44e`, holding 211 parts. `tools/postbuild/` records the tracked successors of the v3.1.0 session (FY27 help text, Luma palette) and later committed-input inserts such as the GST help note. A rebuild from the upstream file still stops at the v3.0.0 artefacts. Byte-for-byte reproduction of the current workbook from upstream is not claimed: the FY27 date shift was Excel-state-dependent and an Excel save is not stable across Excel builds.
